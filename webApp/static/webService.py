...
import webApp.static.ORMService
from webApp.static.coreWebService import add_routes,add_static

def jinjia2_init(app,**kw):
    logging.info("jinjia2 init...")
    options = dict(
    ...
    logging.info("jinjia2 init path %s"%path)
...
def init(loop):
    ...
    jinjia2_init(app,filter=dict(datetime =data_factory))
    add_static(app)
